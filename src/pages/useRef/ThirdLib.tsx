/**
 * 一般第三方图表组件库，第一步都是初始化创建图表实例并关联到对应dom节点上，第二步都是调用api去根据数据去渲染图标
 *
 * 对应在hook中一般都是将关联的dom节点和图表实例都使用useRef进行保存，后续的操作都是通过ref.current保存的唯一图表实例去调用对应的api
 *
...
/**
 * echarts图表都使用echarts-for-react，也能够直接去使用
 */
